...
写请求包含 `device_id`、`expected_revision` 和 `request_id`；完整参数见两个服务的 `tools.json`。查询和操作必须使用同一个服务进程，不能查询后重启进程再提交动作。出口计数与操作前的新查询基准比较，不能使用上传资料中的旧计数作为实时基准。
...
MCP 协议版本为 `2025-06-18`。SQLite 事务对两个进程的查询、版本检查和写入串行化；复位后的版本不复用，旧客户端不能跨轮执行。`manage.py` 是维护方的管理入口，不应作为设备操作工具开放给 Agent；设备操作通过上表中的 MCP 工具完成。
