...
| images.json | file、timestamp、camera_id、view、observation_scope |
...
参考答案、未来工具响应、私有后端状态及数据划分标签不在上传包，也不得进入被评测 Agent 的可读目录或上下文。一个 example 只展示输入格式和联调资料，不提供执行答案。评测端材料位于 `evaluation/grading/`，仅在应用开发完成后由评测端使用，见 `evaluation/README.md`。
